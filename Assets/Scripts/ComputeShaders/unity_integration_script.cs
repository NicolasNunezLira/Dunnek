...
public class SandDuneSimulationGPU2 : MonoBehaviour
{
    ...
    void InitializeComputeShader()
    {
        ...
        // Verificar que el compute shader tenga los kernels
        try
        {
            erosionKernel = sandSimulationShader.FindKernel("ErodeGrains");
            Debug.Log($"ErodeGrains kernel found at index: {erosionKernel}");
        }
        catch (System.Exception e)
        {
            Debug.LogError($"ErodeGrains kernel not found: {e.Message}");
            return;
        }
        ...
        try
        {
            ...
            Debug.Log($"DepositGrains kernel found at index: {depositKernel}");
        }
        catch (System.Exception e)
        {
            Debug.LogError($"DepositGrains kernel not found: {e.Message}");
            return;
        }
        ...
        try
        {
            ...
            Debug.Log($"RunAvalanche kernel found at index: {avalancheKernel}");
        }
        catch (System.Exception e)
        {
            Debug.LogError($"RunAvalanche kernel not found: {e.Message}");
            return;
        }
    }
    ...
    {
        if (!isInitialized || sandSimulationShader == null)
        {
            Debug.LogWarning("Simulation not initialized or shader missing");
            return;
        }
        
        // Verificar que los kernels sean válidos
        if (erosionKernel < 0 || depositKernel < 0 || avalancheKernel < 0)
        {
            Debug.LogError($"Invalid kernels: erosion={erosionKernel}, deposit={depositKernel}, avalanche={avalancheKernel}");
            return;
        }
        ...
        if (windDirection.x != 0 || windDirection.y != 0)
        {
            try
            {
                ...
                Debug.Log($"Dispatching erosion kernel with {threadGroups} thread groups");
                sandSimulationShader.Dispatch(erosionKernel, threadGroups, 1, 1);
                
                Debug.Log($"Dispatching deposit kernel with {threadGroups} thread groups");
                sandSimulationShader.Dispatch(depositKernel, threadGroups, 1, 1);
            }
            catch (System.Exception e)
            {
                Debug.LogError($"Error dispatching erosion/deposit kernels: {e.Message}");
                return;
            }
        }
        ...
        {
            Debug.LogError($"Error dispatching avalanche kernel: {e.Message}");
        }
    }
